...
class Investment:
    ...
    def save_investments(investments):
        with open(config.INVESTMENTS_PATH, "w") as f:
    ...
    def resolve_tier(raw_tier):
        ...
        if raw_tier < 10 or raw_tier > 50:
            return None
        for t in reversed(config.TIERS):
            if raw_tier >= t:
                return t
        return None
        ...
        while True:
            try:
                ...
                for post in topic_data.posts:
                    uid = Investment._extract_user_id(post)
                    if uid:
                        poster_ids.add(uid)
                ...
                print(f"⚠️ Error paginating topic {topic_id} for baseline: {e}")
                break
    ...
    def create_investment(user_id, username, topic_id, invest_amount, tier, time_limit_hours, reward_multiplier):
        ...
        baseline = Investment.get_all_topic_poster_ids(topic_id)
        ...
        investments = Investment.load_investments()
        investments.append(investment)
        Investment.save_investments(investments)
    ...
    def _update_posters(inv):
        ...
        for uid in Investment.get_recent_unique_posters(inv["topic_id"]):
    ...
    def check_investments():
        ...
        for inv in investments:
            ...
            Investment._update_posters(inv)

            if len(inv["new_unique_posters"]) >= inv["tier"]:
                Investment._resolve(inv, db)
                changed = True
