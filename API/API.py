...
class OsuApi:
    ...
    def refresh_access_token():
        ...
        resp = requests.post(config.OSU_TOKEN_URL, data=data)
    ...
    def request_new_token():
        ...
        OsuApi.save_token(token_data)
        return token_data["access_token"]

    @staticmethod
    def get_access_token():
        token = OsuApi.get_cached_token()
        if token:
            return token
        try:
            return OsuApi.refresh_access_token()
        except Exception as e:
            print("Could not refresh token, requesting new one:", e)
            return OsuApi.request_new_token()
    ...
    def get_forum_data(forum_id, access_token):
        ...
        url = config.OSU_FORUM_URL.format(forum_id=forum_id)
        response = requests.get(url, headers=headers)

        # Retry once if token expired
        if response.status_code == 401:
            access_token = OsuApi.request_new_token()
    ...
    def check_new_posts():
        ...
        """
        access_token = OsuApi.get_access_token()
        ...
        forum_data = OsuApi.get_forum_data(config.FORUM_ID, access_token)
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_topic = {executor.submit(OsuApi.get_posts_in_topic, tid): tid for tid in topics_to_fetch}
